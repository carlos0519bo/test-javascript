...
export const ResultPage = () => {
  ...
  const checkIsCorrect = (id: string) => {
    const finded = userAnswers.find((answer) => answer.id === id);
    if (!finded) return "border-transparent";
    return finded?.isCorrect ? "border-green-400" : "border-red-400";
  };
      ...
      {questions.map((question) => (
        <div key={question.title} className='mb-3'>
          ...
          {question.options.map((q) => (
            <div
              className={`items-center justify-between w-full p-5 transition-all rounded-lg mb-3 border-2 ${checkIsCorrect(
                q.id
              )}`}
              key={q.id}
            >
